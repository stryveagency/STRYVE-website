...
function Footer() {
  return (
    <footer className="relative z-2 border-t border-white/8 bg-[#0B0D0E]">
      <div className="mx-auto w-full max-w-[1560px] px-6 py-12 sm:px-10 sm:py-14 lg:px-20 lg:py-15">
        <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-[1.4fr_1fr_1fr_1fr] lg:gap-12">
          <div className="flex flex-col">
            <a href="/#hero" className="flex items-center gap-3 no-underline">
              ...
              <span className="font-body text-2xl leading-none font-light tracking-[0.26em] text-white">
                STRYVE
              </span>
            </a>
            <p className="mt-4 max-w-[240px] font-accent text-base leading-[1.5] text-mint italic">
              Crescimento não é sorte. É processo.
            </p>
          </div>

          <div className="flex flex-col">
            <span className="mb-4 font-body text-xs font-medium tracking-[0.26em] text-gray-light uppercase">
              Contato
            </span>
            <div className="flex flex-col gap-3">
              <a
                ...
                target="_blank"
                rel="noopener"
                className="inline-flex items-center gap-2.5 font-body text-[15px] text-white no-underline transition-colors duration-200 ease-out hover:text-mint"
              >
            ...
            <div className="flex flex-col gap-2.5">
              {navLinks.map((link) => (
                <a
                  key={link.href}
                  href={link.href}
                  className="font-body text-[15px] text-gray-light no-underline transition-colors duration-200 ease-out hover:text-mint"
                >
                  {link.label}
                </a>
              ))}
        ...
        <div className="mt-10 border-t border-white/8 pt-6 text-center sm:text-left">
          <span className="font-body text-xs text-gray-mid">© 2026 STRYVE. Todos os direitos reservados.</span>
        </div>
      </div>
    </footer>
  )
